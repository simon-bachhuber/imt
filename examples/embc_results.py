...
def main(
    ...
    for exp_id in tqdm(range(1, 12), disable=disable_tqdm):
        if one_single_motion:
            motions = [(1, -1)]
        else:
            motions = tqdm(
                filter_motions(_load_timings(exp_id), excludes), disable=disable_tqdm
            )
