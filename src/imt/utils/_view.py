...
def _add_capsule_geoms_to_xml(
    ...
    radius: float,
    color: np.ndarray = np.array([0.5, 0.5, 0.5, 1]),
) -> str:
    """
...
def create_xml_from_graph(graph, positions, skip_0th_body_imu: bool = True):
...
def view(
    ...
    **kwargs,
    ...
    scene = MujocoScene(
        show_stars=show_stars, show_floor=show_floor, floor_z=0.0, **kwargs
    )
    scene.init(geoms)
    ...
    if show_joint_to_joint:
        ...
        xml_string = _add_capsule_geoms_to_xml(xml_string, fromto, 0.01)

    if show_joint_to_imu:
        fromto = {str(i): [] for i in range(len(sys.link_names))}
        for i, p in enumerate(graph):
            if p == -1:
                continue
        ...
        xml_string = _add_capsule_geoms_to_xml(
            xml_string, fromto, 0.007, color=np.array([0.15, 0.56, 0.96, 0.8])
        )
